...
from socialOptimumRegret import socialOptimumInternal, generate_datarate_matrix, normalize_array
...
def finalize_plot(experiment_name, iterations, zero_sum, use_pure_strategies, game, seed, method, ax, game_reps):
    ...
    for name in regrets_to_plot:
        if "Greedy" in name or "Social" in name:
            linewidth = 3
        else:
            linewidth = 1
        if USE_TIME:
...
def full_sweep(game, use_pure_strategies, iterations, method, seed):
    ...
    if plot_dynamic:
        pot_equilibrium, pot_regrets, pot_weights, pot_time = internal_regret(game, seed=seed, use_pure_strategies=use_pure_strategies, iterations=iterations, method=method, dynamic=True, use_potential_function=True)

        print('pot_equilibrium')
    ...
    if plot_socialOptimum:
        ...
        social_equilibrium, social_regrets, social_weights, social_time, social_utility = socialOptimumInternal(game.num_players, game.num_moves, iterations=iterations, datarate=datarate)
        print('social_equilibrium')
        plot_regrets(social_equilibrium, social_regrets, game, step, "Social Optimum", social_time)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--iterations', type=int, default=500, help="Number of iterations to run regret minimization, default 1K")
